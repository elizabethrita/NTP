...
int main(int argc, char * argv[]) {
  ...
  *((char * ) & packet + 0) = 0x1b; // Represents 27 in base 10 or 00011011 in base 2.
  ...
  server = gethostbyname(host_names[0]); // Convert URL to IP.

  if (server == NULL) {
    server = gethostbyname(host_names[1]); // Convert URL to IP.
  }
  ...
  time_t txTm[] = {
    ...
    (time_t)(packet.txTm_s - NTP_TIMESTAMP_DELTA - 25200)//7*60*60 (-7)
  };

  // Print the time we got from the server, accounting for local timezone and conversion from UTC time.

  printf("Time GMT 0:00: %s", ctime((const time_t * ) & txTm[0]));
  printf("Time GMT Palo Alto: %s", ctime((const time_t * ) & txTm[1]));
  return 0;
}